...
export function Hero({
   ...
   * position the sunken-header pages use, so wayfinding sits in one place site-wide.
   */
  crumbs?: readonly Crumb[];
}) {
  return (
    <section className="border-b border-paper-edge bg-paper-sunken">
      <Container width="wide" className={crumbs ? 'pt-6' : undefined}>
        {crumbs && <Breadcrumbs crumbs={crumbs} />}
        <div
          className={cn(
            'grid items-center gap-10 lg:grid-cols-2 lg:gap-16',
            crumbs ? 'pb-14 pt-6 lg:pb-20 lg:pt-10' : 'py-14 lg:py-20',
          )}
        >
          <div>
...
/**
 * Heading-and-body cards, optionally led by a glyph.
 *
 * The icon is opt-in per grid rather than per card: a set where some cards
 * carry one and some do not reads as a set with something missing. Where it is
 * used it gets the same red-on-tint chip the sector cards use, because these
 * two grids are doing the same job — making a wall of similar cards scannable
 * — and two chip treatments for one job is one too many.
 */
export function FeatureGrid({
  items,
}: {
  items: readonly { heading: string; body: string; icon?: ApproachIconName }[];
}) {
  return (
    <ul className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {items.map((item) => (
