 ...
 *
 * Two surfaces ask the same questions — the full form on the contact page and
 * the floating quote chat — and a third (the eventual CRM mapping) will read
 * the same values. The values must match `lib/validation/enquiry.ts` exactly
 * or the server rejects the submission, and the labels must match each other
 * or the same question reads differently depending on where a visitor happens
 * to answer it. Both live here, once.
 *
